    ...
    stage('Run Acceptance Tests') {
      steps {
        script {
          ...
          def retVal = sh(returnStatus: true, script: """export ZAPROXY_HOST=${localIP}; /opt/rh/rh-maven33/root/usr/bin/mvn clean test -Dhr.restapi.url=${REST_API_URL} -Dhr.website.url=${FRONTEND_URL} -Dhr.regular.username=${REGULAR_PASSWORD} -Dhr.regular.password=${REGULAR_USERNAME} -Dtest=RunCukesTest -Dwebdriver.remote.driver=chrome -Dwebdriver.remote.url=http://selenium-hub:4444/wd/hub -Dchrome.switches="--proxy-server=http:${ZAPROXY_HOST}:8080" -Dwebdriver.timeouts.implicitlywait=5000 -Dcukes.config.file=config.properties""")
